    ...
    if(trace->strictly_ordered) {
        if((*(bool (*)(opnd_t))trace->buff->user_data_fill_num)(opnd)) {
            ...
            if(trace->trace_addr || trace->trace_cct) {
                ...
                } else if(!drutil_insert_get_mem_addr(drcontext, bb, instr, opnd, reg_addr/*addr*/,
                        reg_ptr/*scratch*/)) {
...
template<int size>
inline __attribute__((always_inline)) void insertStoreTraceBufferDefault(void *drcontext, instrlist_t *bb, instr_t *instr, opnd_t opnd, vtrace_t *trace, void *user_data)
{
    bool is_float = instr_is_floating(instr);
    int esize = (is_float) ? FloatOperandSizeTable(instr, opnd) : IntegerOperandSizeTable(instr, opnd);
#ifdef VPROFILE_DEBUG
    ...
    if(trace->strictly_ordered) {
        if((*(bool (*)(opnd_t))trace->buff->user_data_fill_num)(opnd)) {
            ...
            if(trace->trace_addr) {
                ...
                vtracer_get_trace_buffer_in_reg(drcontext, instr, bb, trace->buff, reg_ptr);
                if(trace->trace_info) {
                    opnd_t info = OPND_CREATE_INTPTR((*global_client_cb.user_data_cb)(drcontext, instr, bb, opnd, user_data));
...
event_basic_block_default(void *drcontext, void *tag, instrlist_t *bb, bool for_trace, bool translating, OUT void **user_data)
{
    ...
    if(global_client_cb.bb_instrument_cb)
        (*global_client_cb.bb_instrument_cb)(drcontext, bb, &user_info);
    ...
    for (instr = instrlist_first(bb); instr != NULL; instr = instr_get_next(instr)) {
        ...
        if(global_instr_filter(instr)) {
            if(global_client_cb.ins_instrument_cb) {
                (*global_client_cb.ins_instrument_cb)(drcontext, instr, bb, &user_info);
            }
            unsigned int i;
            for (i = 0; i < vtrace_t_list.entries; ++i) {
                vtrace_t *trace = (vtrace_t*)drvector_get_entry(&vtrace_t_list, i);
                ...
                for(int j = 0; j < num; j++) {
                    ...
                    switch(size) {
                        case 1: insertStoreTraceBufferDefault<1>(drcontext, bb, instr, opnd, trace, &user_info);break;
                        case 2: insertStoreTraceBufferDefault<2>(drcontext, bb, instr, opnd, trace, &user_info);break;
                        case 4: insertStoreTraceBufferDefault<4>(drcontext, bb, instr, opnd, trace, &user_info);break;
                        case 8: insertStoreTraceBufferDefault<8>(drcontext, bb, instr, opnd, trace, &user_info);break;
                        case 16: insertStoreTraceBufferDefault<16>(drcontext, bb, instr, opnd, trace, &user_info);break;
                        case 32: insertStoreTraceBufferDefault<32>(drcontext, bb, instr, opnd, trace, &user_info);break;
                        case 64: insertStoreTraceBufferDefault<64>(drcontext, bb, instr, opnd, trace, &user_info);break;
                        case 128: insertStoreTraceBufferDefault<128>(drcontext, bb, instr, opnd, trace, &user_info);break;
                        case 256: insertStoreTraceBufferDefault<256>(drcontext, bb, instr, opnd, trace, &user_info);break;
                        case 512: insertStoreTraceBufferDefault<512>(drcontext, bb, instr, opnd, trace, &user_info);break;
                        default: {
                            // DR_ASSERT_MSG(false, "InstrumentInsCallback Unknown size!");
#ifdef VPROFILE_DEBUG
                            debug_unknown_case(drcontext, instr, opnd);
...
event_basic_block(void *drcontext, void *tag, instrlist_t *bb, bool for_trace, bool translating, OUT void **user_data)
{
    ...
    // client cb
    if(global_client_cb.bb_instrument_cb)
        (*global_client_cb.bb_instrument_cb)(drcontext, bb, NULL);

    if(global_client_cb.ins_instrument_cb) {
        for (instr = instrlist_first(bb); instr != NULL; instr = instr_get_next(instr)) {
            if(!instr_is_app(instr) || instr_is_ignorable(instr)) continue;
            if(global_instr_filter(instr)) {
                (*global_client_cb.ins_instrument_cb)(drcontext, instr, bb, NULL);
            }
        }
    }
...
void vprofile_register_client_cb(void* (*user_data_cb)(void *, instr_t *, instrlist_t *, opnd_t, void*),
                                 void (*ins_instrument_cb)(void *, instr_t *, instrlist_t *, void*),
                                 void (*bb_instrument_cb)(void *, instrlist_t *, void*))
{
                   ...
                   uint8_t flag)
{
    ...
    if(flag == VPROFILE_DEFAULT) {
        if(!drvector_init(&vtrace_t_list, 1, false /*!synch*/, NULL) ||
           !drmgr_register_bb_instrumentation_event(event_basic_block_default, NULL, NULL)) {
            DR_ASSERT_MSG(false, "ERROR: vprofile unable to register events");
        }
        return true;
    }

    drmgr_priority_t drmgr_bb_high_pri = { sizeof(drmgr_bb_high_pri),
                                         "vprofile-drmgr-bb-event", NULL, NULL,
                                         DRCCTLIB_THREAD_EVENT_PRI - 5 };

    if(!drvector_init(&vtrace_t_list, 1, false /*!synch*/, NULL) ||
       !drmgr_register_bb_instrumentation_event(event_basic_block, NULL, &drmgr_bb_high_pri)) {
           DR_ASSERT_MSG(false, "ERROR: vprofile unable to register events");
    }
