...
public:
    ...
    L1Cache(uint32_t size_kb, L1Type type, ShadowPageTable* spt, file_t);
    ~L1Cache();

    // test
    void print_here(val_info_t *info);
    
    void load(uint64_t addr, uint64_t pc, int32_t read_bytes, int32_t tid, bool check_sharing);
    void store(uint64_t addr, uint64_t pc, int32_t write_bytes, int32_t tid);
...
private:
    ...
    int64_t l1_coherence_miss_ts_cnt;
    int64_t l1_coherence_miss_fs_cnt;
    int64_t l1_capacity_miss_cnt;
    int64_t l1_conflict_miss_cnt;
    
    
    std::unordered_map<uint64_t, int64_t> pc_coherence_miss_map_ts; // true sharing
    std::unordered_map<uint64_t, int64_t> pc_coherence_miss_map_fs; // false sharing
    std::unordered_map<uint64_t, int64_t> pc_capacity_miss_map;
    std::unordered_map<uint64_t, int64_t> pc_conflict_miss_map;
    ...
    void print_miss_cnt_topk(int topk, std::unordered_map<uint64_t, int64_t>& pc_miss_map, CacheMissReason cache_reason, int32_t tid);
