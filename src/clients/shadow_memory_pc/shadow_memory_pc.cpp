...
#define shadow_memory_EXIT_PROCESS(format, args...)                            \
    DRCCTLIB_CLIENT_EXIT_PROCESS_TEMPLATE("shadow_memory_pc", format, ##args)
...
std::atomic<int32_t> thread_id{0};
int win_enable;
int win_disable;
...
void ins_handler(L1Cache* l1d, val_info_t *info, per_thread_t* pt){
    ...
    if ((info->type) & (vprofile_src_t::MEMORY)){
        ...
        if(((info->type) & (vprofile_src_t::READ)) && ((info->type) & (vprofile_src_t::BEFORE))){
            ...
            l1d->load(info->addr, info->pc, (info->size * info->esize), tid, true);
            ...
            // drcctlib_print_backtrace(pt->output_file, info->ctxt_hndl, false, true, 50 /*MAX_CCT_DEPTH*/);
            l1d->store(info->addr, info->pc, (info->size * info->esize), tid);
...
ThreadOutputFileInit(per_thread_t *pt, void *drcontext)
{
    ...
    int32_t id = thread_id.fetch_add(1, std::memory_order_relaxed);
    printf("id %d\n", id);
    pt->threadId = id;
    ...
    if (op_enable_sampling.get_value()) {
        dr_fprintf(pt->output_file, "[shaodw_memory_pc INFO] Sampling Enabled\n");
    } else {
        dr_fprintf(pt->output_file, "[shaodw_memory_pc INFO] Sampling Disabled\n");
    }
}
...
ClientThreadStart(void *drcontext)
{
    ...
    pt->memory_ins_cnt=0;
    pt->total_load_cnt=0;
    pt->total_store_cnt=0;
    pt->l1d = new L1Cache(L1_CACHE_SIZE, L1Type::DATA, spt, pt->output_file);
}
...
ClientInit(int argc, const char *argv[])
{
    ...
    sprintf(name + strlen(name), "-%d-shadow_memory_pc", pid);
    g_folder_name.assign(name, strlen(name));
    mkdir(g_folder_name.c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);

    dr_fprintf(STDOUT, "[shadow_memory_pc INFO] Profiling result directory: %s\n", g_folder_name.c_str());

    sprintf(name+strlen(name), "/shadow_memory_pc.log");
    ...
    if (op_enable_sampling.get_value()) {
        dr_fprintf(STDOUT, "[shadow_memory_pc INFO] Sampling Enabled\n");
        dr_fprintf(gFile, "[shadow_memory_pc INFO] Sampling Enabled\n");
        win_enable = op_window_enable.get_value();
        win_disable= op_window.get_value();
        float rate = (float)win_enable / (float)win_disable;
        dr_fprintf(STDOUT, "[shadow_memory_pc INFO] Sampling Rate: %.3f, Window Size: %ld\n", rate, win_disable);
        dr_fprintf(gFile,  "[shadow_memory_pc INFO] Sampling Rate: %.3f, Window Size: %ld\n", rate, win_disable);
    } else {
        dr_fprintf(STDOUT, "[shadow_memory_pc INFO] Sampling Disabled\n");
        dr_fprintf(gFile, "[shadow_memory_pc INFO] Sampling Disabled\n");
    }
    if (dr_using_all_private_caches()) {
        dr_fprintf(STDOUT, "[shadow_memory_pc INFO] Thread Private is enabled.\n");
        dr_fprintf(gFile,  "[shadow_memory_pc INFO] Thread Private is enabled.\n");
    } else {
        dr_fprintf(STDOUT, "[shadow_memory_pc INFO] Thread Private is disabled.\n");
        dr_fprintf(gFile,  "[shadow_memory_pc INFO] Thread Private is disabled.\n");
    }
...
ClientExit(void)
{
    ...
    if (!dr_raw_tls_cfree(tls_offs, INSTRACE_TLS_COUNT)) {
        shadow_memory_EXIT_PROCESS(
            "ERROR: shaodw_memory_pc dr_raw_tls_cfree fail");
    }
    ...
    if (!drmgr_unregister_thread_init_event(ClientThreadStart) ||
        ...
        printf("ERROR: shaodw_memory_pc failed to unregister in ClientExit");
        fflush(stdout);
        exit(-1);
    }
...
{
    dr_set_client_name("DynamoRIO Client 'shadow_memory_pc'",
