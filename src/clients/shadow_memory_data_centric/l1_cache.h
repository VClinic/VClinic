...
public:
    ...
    void load(uint64_t addr, data_handle_t dobj, int32_t read_bytes, int32_t tid);
    void store(uint64_t addr, data_handle_t dobj, int32_t tid);
...
private:
    ...
    std::unordered_map<std::string, int64_t> dobj_coherence_miss_map;
    std::unordered_map<std::string, int64_t> dobj_capacity_miss_map;
    std::unordered_map<std::string, int64_t> dobj_conflict_miss_map;
    ...
    int addr_size;
    std::vector<uint64_t> least_addr_list;
    std::vector<int32_t> least_size_list;
    ...
    void print_miss_cnt_topk(int topk, std::unordered_map<std::string, int64_t>& cct_miss_map, CacheMissReason cache_reason);
