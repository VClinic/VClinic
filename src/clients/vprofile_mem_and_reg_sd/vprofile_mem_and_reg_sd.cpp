...
VPROFILE_FILTER_OPND(opnd_t opnd, vprofile_src_t opmask) {
    uint32_t user_mask = (ANY_DATA_TYPE | REGISTER | MEMORY | READ | WRITE | BEFORE | AFTER);
...
{
    dr_set_client_name("DynamoRIO Client 'vprofile_mem_and_reg_sd'",
    ...
    vtrace = vprofile_allocate_trace(VPROFILE_TRACE_DEFAULT | VPROFILE_TRACE_STRICTLY_ORDERED | VPROFILE_TRACE_BEFORE_WRITE | ex_trace_flag);

    uint32_t opnd_mask = (ANY_DATA_TYPE | REGISTER | MEMORY | READ | WRITE | BEFORE | AFTER);
